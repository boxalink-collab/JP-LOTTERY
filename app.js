...
const firebaseConfig = {
  apiKey: "AIzaSyA1Rc2pEVjxnyctC6UIVRYfYuRY3UCuauk",
  authDomain: "jp-lottery.firebaseapp.com",
  projectId: "jp-lottery",
  storageBucket: "jp-lottery.firebasestorage.app",
  messagingSenderId: "75869862960",
  appId: "1:75869862960:web:1442a66fcfd440687301f5"
};
...
function init() {
  ...
  // Accueil public toujours affiché — auth seulement pour les actions
...
function showGuest() {
  ...
  if (authBtn) { authBtn.textContent = '🔐'; authBtn.title = 'Se connecter'; }
  updBal();
  renderCC();
  renderLS();
  renderRes();
...
// Ouvre auth ou profil selon état connexion
...
// Appeler avant toute action nécessitant connexion
// action = string JS à eval si connecté, showLogin = ouvrir onglet login
function requireAuth(action, showLogin) {
  if (S.user) {
    ...
    openS('authSh');
    setATab('l');
    toast('Connectez-vous pour effectuer cette action', '🔐', 'inf');
  }
}

// Ferme le modal auth (uniquement si clic sur l'overlay lui-même)
...
function enterApp() {
  ...
  var authBtn = document.getElementById('authBtn');
  if (authBtn) { authBtn.textContent = '👤'; authBtn.title = 'Mon profil'; }
  ...
  var ghello = document.getElementById('ghello');
  if (ghello) ghello.textContent = 'Bonjour 👋';
  var gname  = document.getElementById('gname');
...
function doLogout() {
  ...
  S.tickets = [];
  S.hist = [];
  auth.signOut(); // déclenche onAuthStateChanged → showGuest()
}
...
// Écrans nécessitant connexion : 1=Parier, 3=Portefeuille, 4=Tickets
var PROTECTED = [1, 3, 4];

function goScr(idx) {
  // Bloquer les écrans protégés si non connecté
...
// =========================================================
function placeBet() {
  if (!S.user) { openS('authSh'); setATab('l'); toast('Connectez-vous pour parier','🔐','inf'); return; }
